...
(function () {
    ...
    function beforeRemove(node, index, item) {
        ...
        if (!$.data(node, 'isotope-sort-data')) {
            logger.error('Not an Isotope element:', node);
            throw new Error('Not an Isotope element');
        }
    ...
    ko.bindingHandlers.isotope = {
        ...
        'update': function(element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) {
            ...
            if (!haveInitialized) {
                ...
                logger.debug('Binding update called for the first time, initializing Isotope, options:', isotopeOptions);
                
                // Elements are added to the DOM, now initialize Isotope
                $container.isotope(isotopeOptions);
                console.log('Elements and their sort data:');
                $container.find('.isotope-item').each(function (index, elem) {
                    logger.debug(elem, $.data(elem, 'isotope-sort-data'));
                });
            }
            else {
